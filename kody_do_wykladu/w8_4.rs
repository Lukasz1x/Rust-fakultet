#[derive(PartialEq, Copy, Clone)]
enum Element {
    Plus,
    Minus,
    Razy,
    Podzielic,
    Potega,
    Liczba(f64)
}

use crate::Element::{Plus, Minus, Razy, Podzielic, Potega, Liczba};     // przemyśleć trzeba, czy konieczne
// use crate::Element::*;       // niby to samo, ale duuuużo gorsze

type Stos = Vec<f64>;

fn wykonaj_dzialanie(rodzaj: Element, a: f64, b: f64) -> f64 {
    match rodzaj {
        Plus       => a + b,
        Minus      => a - b,
        Razy       => a * b,
        Podzielic  => a / b,
        Potega     => a.powf(b),
        Liczba(_)  => panic!("niespodziewana Liczba (z typu Element)")
    }
}

fn oblicz_onp(kolejka: &Vec<Element>) -> Option<f64> {
    let mut stos = Stos::new();
    for e in kolejka {
        if let Liczba(wartosc) = *e {
            stos.push(wartosc);
        } else {
            let b = stos.pop()?;
            let a = stos.pop()?;
            let wynik = wykonaj_dzialanie(*e, a, b);
            stos.push(wynik);
        }
    }
    let wynik_koncowy = stos.pop();
    if !stos.is_empty() {
        return None;
    }
    return wynik_koncowy;
}

fn main() {
    let mut k = Vec::new();
    k.push(Liczba(34.5));
    println!("{:?}", oblicz_onp(&k));
    k.push(Liczba(1.0));
    k.push(Liczba(0.5));
    k.push(Plus);
    k.push(Razy);
    println!("{:?}", oblicz_onp(&k));

    let k1 = Vec::new();
    println!("{:?}", oblicz_onp(&k1));

    let mut k2 = Vec::new();
    k2.push(Liczba(1.0));
    k2.push(Liczba(0.5));
    println!("{:?}", oblicz_onp(&k2));  // None

    let mut k3 = Vec::new();
    k3.push(Liczba(1.0));
    k3.push(Liczba(0.5));
    k3.push(Plus);
    k3.push(Plus);
    k3.push(Plus);
    println!("{:?}", oblicz_onp(&k3));  // None

    let mut k4 = Vec::new();
    k4.push(Liczba(2.0));
    k4.push(Liczba(0.5));
    k4.push(Potega);
    println!("{:?}", oblicz_onp(&k4));
}
